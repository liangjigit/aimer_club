	...
	<view class="wrapper" v-if="isShow" @touchmove.stop.prevent="moveHandle">
		<view class="bg-mask flex flex-direction align-center justify-center">
			<view class="card flex flex-direction align-center" @click="savePhoto(posterImg)">
				<view class="btn">
					<canvas class="canvas" :style="{width:pupopWidth +'rpx',height: pupopHeight + 'rpx'}"
						canvas-id="guidePopop" id="guidePopop" @error="canvasError"></canvas>
				</view>
			</view>
			...
			<view @click="hide" class="cancel-btn" v-if="showCloseBtn">
				<image class="image1" src="/static/close-icon.png" mode="aspectFit"></image>
			</view>
		</view>
	</view>
</template>
					...
					success(image) {
						...
						uni.getImageInfo({
							...
							success(res) {
								...
								ctx.beginPath(); //开始绘制
								//先画个圆
								ctx.arc(uni.upx2px(acr_x), uni.upx2px(acr_y), uni.upx2px(acr_r), 0, Math
									.PI * 2);
								ctx.setFillStyle('#ffffff')
								ctx.fill() //保证图片无bug填充
								ctx.clip(); //画了圆 再剪切 原始画布中剪切任意形状和尺寸。一旦剪切了某个区域，则所有之后的绘图都会被限制在被剪切的区域内
								ctx.drawImage(res.path, uni.upx2px(img_x), uni.upx2px(img_y), uni.upx2px(
									img_w), uni.upx2px(img_h)) // 头像
								ctx.restore();
								// 绘制 昵称
								// ctx.save(); // 先保存状态 已便于画完圆再用
								// const text_size = 28
								...
								uni.getImageInfo({
									...
									success(res) {
										...
										const img_w = size
										const img_h = img_w
										ctx.drawImage(res.path, uni.upx2px(img_x), uni.upx2px(
											img_y), uni.upx2px(img_w), uni.upx2px(img_h)) // 头像
										ctx.draw(false, () => {
