	...
	<view class="flex flex-direction align-center">
		...
		<view class="poster">
			...
			<canvas class="canvas" :style="{width:screenWidth+'px',height: screenHeight + 'px'}" canvas-id="invitePoster" id="invitePoster" v-else  @error="canvasError"></canvas>
			 -->
			<canvas class="canvas" width="750" height="1334" :style="{width:canvasWidth+'px',height: posterImg?0:canvasHeight + 'px'}" canvas-id="invitePoster" id="invitePoster"  @error="canvasError">
			</canvas>
			<image :src="posterImg" mode="widthFix" :style="{width:'100%',height:canvasHeight  + 'px'}" v-if="posterImg" ></image>
...
<script>
	...
	import {doActivityStatistics,doBrartsStatistics,doCourseStatistics,doCupStatistics} from '@/common/statistics.js'
	...
	export default{
		...
		onLoad(options){
			let {type,poster} = this.shareInfo
					...
					if(_this.shareInfo.type == Type[1]){ //邀请好友
						uni.getImageInfo({
							...
							success(res){
								...
								ctx.drawImage(res.path,uni.upx2px(260),uni.upx2px(965),uni.upx2px(240),uni.upx2px(240)) // 小程序码
								ctx.draw(false, () => {
		...
		methods:{
			...
			setStatistics(type){
				let {statistics,code} = this.shareInfo
				let no = this.getDownloadPosterNo(type);
				let param = {modelCode:code,no}
				switch(statistics){
					case "activity":
