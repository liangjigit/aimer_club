import {GETREDIRECTURL,GETUSERINFO,CHANGELOGINSTATE,GETLOGINPOPUP,SETSHOWACTIVITY,GETPOPUP,GETGUIDINFO,GETINVITEUSERID,GETUSERACCOUNTSTATE} from '@/store/mutations_type.js'
...
const actions = {
	// 保存手机号
	async savePhoneNumber({state,commit,dispatch},payload){
	...
	async login({commit,dispatch,state},payload){
		...
		if(response.code == 200){
			let { token, getPhone, sessionKey } = response.data; // getPhone, sessionKey, 
	...
	async getMyInfo({state,commit},payload){
		...
		if(response.code == 200){
			let {currentIntegral,showActivity,level,memberNo,id,guideId,isGuide,phone,isDisabledUser} = response.data
			if(!payload.userInfo) payload.userInfo = state.userInfo
			commit('GETUSERINFO',{userInfo:{...payload.userInfo,currentIntegral,level,memberNo,id,guideId,isGuide,phone,isDisabledUser}})
			commit('SETSHOWACTIVITY',{showActivity})
			commit('GETUSERACCOUNTSTATE',{isDisabledUser})
		}
		return response;
	},
	// 获取登录弹窗
	async getLoginPopup({commit},payload){
		let response = (await request({
		...
		if(response.code == 200){
			commit('GETPOPUP',{pupop:response.data})
		}
		return response;
	},
	// 获取企业登录凭证。
	// 当用户在企业微信中打开小程序时，需要获取企业微信登录凭证js_code，后端去通过jscode 和 access_token 获取 userId ,再通过 userId 和 access_token 读取企业员工信息中的手机号，与微信小程序用户授权获取的手机号进行匹配，若匹配上了则为导购人员。
	wxworkLogin({commit},payload){
	...
	// 导购关系绑定
	async bindWithGuid({commit,dispatch,state},payload){
		const {guidecode,inviteUserId} = state // 导购信息
		let response = (await request({ 
		...
		if(response.code == 200){
			// 获取个人资料
			await dispatch('getMyInfo', {userInfo: state.userInfo})
			commit('GETINVITEUSERID',{inviteUserId:null})
			commit('GETGUIDINFO',{guidecode:null,source:null,aimerid:null,erpcode:null,appid:null})
		}
		return response
	}
}
...
const mutations = {
	...
	[GETUSERINFO](state,payload){
		let userInfo = {...state.userInfo,...payload.userInfo}
