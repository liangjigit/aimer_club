import {SETSELECTOR,GETBANNERS,GETNAVS,GETMINIPROGRAMSHARE,TRAININGCOUNT} from '@/store/mutations_type.js'
...
const actions = {
	...
	async getNavData({commit},payload){
		...
		if(response.code == 200){
			const {imageInfo,menuList,windowList} = response.data
			commit('GETNAVS',{imageInfo,menuList,windowList})
		}
		return response;
	},
	// 首页CUP有料专家列表
	...
	// 小程序分享
	async getShareContent({commit},payload){
		let response = (await request({
		...
		if(response.code == 200){
			commit('GETMINIPROGRAMSHARE',{share:response.data})
		}
		return response;
	},
	// 首页 加入训练人数
	async getTrainingCount({commit},payload){
		let response = (await request({
		...
		if(response.code == 200){
			...
			commit('TRAININGCOUNT',{userCount})
		}
		return response;
	},
	// 获取导购海报
	async getGuidePoster({commit},payload){
		let response = (await request({
	...
	// 获取直播间跳转地址
	async getLiveRoom({commit},payload){
		let response = (await request({
