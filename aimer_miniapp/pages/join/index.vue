	...
	<view>
		...
		<view>
			<view class="bg-image">
				...
				<image src="https://aimer-zt.oss-cn-beijing.aliyuncs.com/pictures_test/1603965045021.jpg"
			...
			<view class="bg-mask flex flex-direction justify-end" style="padding-top: 140upx;">
				...
				<view class="flex justify-center" style="margin-top: 52upx;">
					<view class="coupon flex flex-direction justify-center" v-for="(item,index) in coupons"
						:key="index">
				...
				<form>
						...
						<view class="write-content flex align-center flex-wrap">
							<block v-for="(item,index) in fieldList" :key="item.id">
								<view class="flex item" v-if="item.field == 'realname'">
									<input class="flex-sub" type="text" @input="onInput(index,$event)"
										:value="item.value"
										:placeholder="'您的'+ item.fieldName + (item.isRequired ==1 ?'' : '(选填)')"
										placeholder-style="color:#fff" />
								</view>
								<picker class="item" v-if="item.field == 'birthday'|| item.field == 'childrenBirthday'"
									@change="change(index,$event)" v-model="item.value" mode="date" :start="start"
									:end="end">
									<view class="flex align-center">
										<text class="flex-sub input"
											v-if="item.field == 'birthday'">{{item.value||('您的' + item.fieldName + (item.isRequired ==1 ?'' : '(选填)'))}}</text>
										<text class="flex-sub input"
											v-if="item.field == 'childrenBirthday'">{{item.value|| (item.fieldName + (item.isRequired ==1 ?'' : '(选填)'))}}</text>
										<text class="cuIcon-playfill text-white"></text>
									</view>
								</picker>
								<picker class="item" v-if="item.field == 'gender'" @change="change(index,$event)"
									v-model="item.value" :range="GenderRange" range-key="name">
									<view class="flex align-center">
										<text
											class="flex-sub input">{{GenderRange[item.value].name || ('您的' + item.fieldName + (item.isRequired ==1 ?'' : '(选填)'))}}</text>
										<text class="cuIcon-playfill text-white"></text>
									</view>
								</picker>
							</block>
					...
					<view class="flex flex-direction align-center" style="width: 100%;">
						<button class="cu-btn round" open-type="getPhoneNumber" v-if="!isSavePhone"
							@getphonenumber="getPhoneNumber">立即领取</button>
						<button class="cu-btn round" v-else @click="submit">立即领取</button>
						<button class="other" open-type="getPhoneNumber"
							@getphonenumber="getPhoneNumber">使用其他手机号登录领取</button>
						<view class="agree-box flex align-center justify-center" @click.stop="changeAgree">
							<image src="/static/join/checkbox-icon.png" class="checkbox" mode="aspectFit"
								v-show="!isAgree"></image>
							<image src="/static/join/cheked-icon.png" class="checkbox" mode="aspectFit"
								v-show="isAgree"></image>
							<text>我已阅读并同意</text>
