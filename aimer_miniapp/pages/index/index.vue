<template>
	<view class="">
		...
		<view class="loginBox flex align-center" v-else-if="userInfo" >
			<image :src="userInfo.avatarUrl||'/static/default-avatar.png'" mode="aspectFill" @click="showGuidePopup"></image>
		...
		<view class="flex justify-center" v-if="banners.length>0" > 
			<swiper class="banner card-swiper" next-margin="100rpx" :circular="true" :interval="5000" :duration="500" :autoplay="true" easing-function="linear" @change="bannerSwiper">
		...
		<view class="cu-list grid col-4 no-border" v-if="menuList.length>0" >
		    <view class="cu-item align-center" v-for="(item,index) in menuList" :key="index" @click="linkToPage(item,0)">
		      <view class="icon">
		 ...
		 <view class="image-link text-center" v-if="singleImage">
		  ...
		  <view class="flex justify-center" style="padding-top: 22upx;">
			 ...
			 <view class="right-block flex flex-direction justify-between">
			 	 <view class="btn flex flex-direction justify-center align-center" v-for="(item,index) in windowList" :key="index" @click="linkToPage(item,1)">
		  ...
		  <view class="flex justify-center">
			<swiper class="tutor card-swiper" previous-margin="230rpx" next-margin="230rpx" :interval="3000" :duration="500" :circular="true" :autoplay="true" @change="tutorSwiper">
		  ...
		  <scroll-view scroll-x="true" enable-flex="true" style="height:106upx" class="scroll-view_H">
		  	<button class="cu-btn round line-black margin-right-sm" :class="{current:selectedTagsId == 0}" @click="changeTags(0)">全部</button>
		  	<button class="cu-btn round line-black margin-right-sm" :class="{current:selectedTagsId == item.id}" @click="changeTags(item.id)" v-for="(item,index) in tags" :key="item.id" >{{item.tagsName}}</button>
		  </scroll-view>
...
<script>
	...
	import {doBannerStatistics,doNavStatistics} from '@/common/statistics.js'
	import {guidePopup} from '@/pages/components/popup/guide-popup.vue'
	...
	export default {
		...
		computed:{
			...mapState('index',['selector','banners','singleImage','menuList','windowList','miniProgramShare','trainingCount']),
		...
		onLoad(options) {
			...
			const {guidecode,source,aimerid,erpcode,appid,inviteUserId,scene} = options
			const userId = inviteUserId||scene
			if(guidecode||source||aimerid||erpcode||appid||userId){
				this.GETGUIDINFO({guidecode,source,aimerid,erpcode,appid})
				this.GETINVITEUSERID({inviteUserId:userId})
			}
		...
		methods: {
			...
			...mapActions('index',['getBanners','getNavData','getExperts','getTags','getShareContent','getTrainingCount']),
			...
			getData(isRefresh){
				this.getBanners({pageId:1})// 1 表示首页
				this.getNavData({})
