	...
	<view class="wrapper" v-show="isShow&&pupop" @touchmove.stop.prevent="moveHandle">
		<view class="bg-mask flex align-center justify-center">
			<view class="card flex flex-direction align-center">
				<button open-type="getUserInfo" v-if="pupop.image&&!isDefault" @getuserinfo="login" withCredentials="false" class="btn">
				...
				<view class="bg-white flex flex-direction align-center" v-else>
					...
					<button open-type="getUserInfo" @getuserinfo="login" withCredentials="false" class="cu-btn line-red text-red">登录</button>
				</view>
...
<script>
	...
	import {doPupopClickStatistics} from '@/common/statistics.js'
	export default{
		props:{
			isDefault:Boolean,
			defaultTitle:{
		...
		methods: {
			...
			login(e){
				const {id} = this.pupop
				if(id){
					doPupopClickStatistics({id}) // 数据统计
				}
				let { errMsg } = e.detail
					...
					this.onGetUserInfo().then((response)=>{
						if(response.code == 200){
							...
							if(this.redirectUrl){ // 判断需要重定向的页面是否是当前页
								...
								const {linkType,linkUrl,miniappId,displayPage} = this.pupop
