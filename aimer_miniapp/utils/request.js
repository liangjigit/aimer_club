...
let request = axios.create({
  baseURL: process.env.NODE_ENV === 'development'?"https://clubtest.aimer.com.cn":"https://aimerclub.aimergroup.com", //baseApi[env]||
  headers: {
...
axiosRetry(request, {
  ...
  retryCondition: async (error) => {
	...
	if(error.message.includes("status code 401")){
		await store.dispatch("login/onGetUserInfo",{},{root:true});
		return true
	...
	uni.getSetting({
		success: async function(t) {
			if(!t.authSetting["scope.userInfo"]){
				...
				store.commit("login/CHANGELOGINSTATE",{loginState:false},{root:true})
				if(!store.state.login.isShowLogin){
					store.commit("login/GETLOGINPOPUP",{},{root:true})
				}
				uni.showToast({
					title: '登录已过期，请重新登录',
					icon: 'none',
